...
class ExchangeScreen extends React.Component {
  ...
  drop1_open = (flag) => {
    if (flag) this.setState({drop1_open_flag: flag, drop2_open_flag: false});
    else this.setState({drop1_open_flag: flag});
  };
  ...
  buyInputChange = (e, exchage_from_data) => {
    ...
    let receiveInputValue1 = (
      (e * exchage_from_data[drop1_key]['price']) /
      price.atri
    ).toFixed(0);
    if (
      (e * exchage_from_data[drop1_key]['price']) / price.atri > 0 &&
      (e * exchage_from_data[drop1_key]['price']) / price.atri < 1
    ) {
  ...
  receiveInputChange = (e1, exchage_from_data) => {
    ...
    ).toFixed(exchage_from_data[drop1_key]['decimal']);
  ...
  usdInputChange = (e, exchage_from_data) => {
    ...
    let buyInputValue1 = (e / exchage_from_data[drop1_key]['price']).toFixed(
      exchage_from_data[drop1_key]['decimal'],
    );
  ...
  render() {
    ...
    const exchage_from_data = [
      // {image:Images.Atri_icon,value:balance.atri.toFixed(4),u_v:(balance.atri_usd).toFixed(2),f_text:'ATRI', text:'Atari token',price:price.atri,decimal:4},
      {
        ...
        value: balance?.eth?.toFixed(8),
        u_v: balance?.eth_usd?.toFixed(2),
        f_text: 'ETH',
        text: 'Ethereum',
        price: price?.eth,
        decimal: 8,
      ...
      {
        image: Images.bch_icon,
        value: balance?.usdt?.toFixed(6),
        u_v: balance?.usdt_usd?.toFixed(2),
        f_text: 'USDT',
        text: 'USDT',
        price: price?.usdt,
        decimal: 8,
      },
      {
        ...
        value: balance?.btc?.toFixed(8),
        u_v: balance?.btc_usd?.toFixed(2),
        f_text: 'BTC',
        text: 'Bitcoin',
        price: price?.btc,
        decimal: 8,
      ...
      {
        image: Images.bnb_icon,
        value: balance?.bnb?.toFixed(8),
        u_v: balance?.bnb_usd?.toFixed(2),
        f_text: 'BNB',
        text: 'BNB',
        price: price?.bnb,
        decimal: 8,
      ...
      {
        image: Images.Ltc_icon,
        value: balance?.ltc?.toFixed(8),
        u_v: balance?.ltc_usd?.toFixed(2),
        f_text: 'LTC',
        text: 'Litecoin',
        price: price?.ltc,
        decimal: 6,
    ...
    const exchage_from_data1 = [
      {
        image: Images?.Atri_icon,
        value: balance?.atri.toFixed(0),
        u_v: (balance?.atri_usd).toFixed(2),
        f_text: 'ATRI',
        text: 'Atari token',
        price: price?.atri,
        decimal: 0,
      ...
      <View style={[CustomStyles.container, styles.innerContainer]}>
        ...
        <View
          ...
          <ExchangeInput
            label={'Buy now'}
            onChangeInput={(e) => {
              this.buyInputChange(e, exchage_from_data);
            }}
            onChangeUsdInput={(e) => {
              this.usdInputChange(e, exchage_from_data);
            }}
          ...
          <ExchangeInput
            label={'Receive'}
            onChangeInput={(e) => {
              this.receiveInputChange(e, exchage_from_data);
            }}
            onChangeUsdInput={(e) => {
              this.usdInputChange(e, exchage_from_data);
            }}
    ...
    return (
      <ImageBackground
        ...
        <Modal isVisible={this.state.show_modal}>
          <View style={{backgroundColor: 'white', borderRadius: 10}}>
            <Image
              source={Images.exchange_gif}
              style={{
                justifyContent: 'center',
                width: '100%',
                height: 200,
            ...
            <Text style={{textAlign: 'center', padding: 20, fontSize: 20}}>
              Exchange feature will be available soon.
            </Text>
            <TouchableOpacity
...
export default connect(mapStateToProps, {updateBallance})(
  withTheme(ExchangeScreen),
);
