...
class SendPaymentScreen extends React.Component {
  ...
  currencyCheck = async () => {
    ...
    let r_address;
    let currency = 0;
    if (address[0] == 'a' && address[1] == 't') currency = 1;
    else {
      r_address = this.getAddress(address);
      if (WAValidator.validate(r_address, 'BTC')) currency = 0;
      else if (WAValidator.validate(r_address, 'LTC')) currency = 3;
      else if (WAValidator.validate(r_address, 'ETH')) currency = 2;
      else currency = 4;
    }
    const current_usd_val = await currency_convertApi(
      Headers[currency]['text'].toLowerCase(),
      1,
    );
    this.setState({
  ...
  SendConfirm = () => {
    ...
    if (r_address !== '' && send_amount !== '0.00') {
      let info = {
        send_usd_amount: send_usd_amount,
  ...
  changeSendUsdValue = (e) => {
    ...
    let send_amount1 =
      e !== ''
        ? (
            parseFloat(e) / parseFloat(price[currency_data[currency][0]])
          ).toFixed(5)
        : '0.00';
    this.setState({
  ...
  getAddress = (address) => {
    ...
    let r_address = '';
    if (split.length > 1) r_address = split[1];
    else r_address = split[0];
    return r_address;
  };
  setFullBallance = () => {
    const {balance, currency} = this.state;
    const currency_data1 = [
      ['btc', balance.btc, '#f7931a', balance?.btc_usd],
    ...
    return (
      <SafeAreaView
        ...
        <View
          ...
          <View
            ...
            <View
              ...
              <View
                ...
                <Text
                  style={{color: darkmode ? 'white' : 'black', fontSize: 24}}>
                  {Headers[currency]['text']}
                </Text>
              </View>
              <View style={{width: '10%'}}>
                <Ionicons
            ...
            <View
              ...
              <Text
                ...
                *Available: {currency_data[currency][1]}{' '}
                {Headers[currency]['text']}
              </Text>
              <TouchableOpacity
                style={{
            ...
            <View
              ...
              <TextInput
                ...
                onChangeText={(key) => this.setState({r_address: key})}
              ...
              <TouchableOpacity
                onPress={() => this.setState({qr_code_modal: true})}
                style={{marginTop: 20}}>
                <Ionicons
          ...
          <Modal
            isVisible={this.state.qr_code_modal}
