...
export default class PTRView extends React.Component {
  ...
  _onScrollEvent(e) {
    ...
    if (e.nativeEvent.velocity.y <= 0) {
      const scrollable =
        layoutScrollHeight > height ? layoutScrollHeight : height;
  ...
  startLoadingAnim() {
    ...
    Animated.timing(this.spinAnim, {
      ...
      if (this.state.shouldTriggerRefresh) this.startLoadingAnim();
    });
  }
  ...
  render() {
    const {isScrollFree, minHeight, refreshHeight, minPullDistance} =
      this.state;
