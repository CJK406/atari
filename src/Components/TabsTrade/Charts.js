...
const Charts = (props) => {
  ...
  const getGraphData = async () => {
    ...
    const graph_percent = await get_percent(
      tabData?.graph_text,
      percent_data[currTabPeriod][0],
    );
    try {
      ...
      for (var i = 0; i < graph_data?.length; i++) {
        ...
        if (graph_data[i][4] < min) min = graph_data[i][4];
        if (graph_data[i][4] > max) max = graph_data[i][4];
      }
      _data.min = min;
      _data.max = max;
  ...
  const perTabLabel = {1: '1D', 7: '7D', 30: '1M'};
  const chartConfig = {
  ...
  const periodeTab = (period) => {
    return (
      <TouchableOpacity
        key={period}
        onPress={() => setCurrTabPeriod(period)}
        style={[
